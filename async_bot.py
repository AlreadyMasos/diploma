...
async def start(message: aiogram.types.Message):
    await message.reply('''Введите команду /train, если хотите начать тренировку''')
...
async def video(message: aiogram.types.Message):
    ...
    pu = reworker.all_track(video.name)[0]
    sq = reworker.all_track(video.name)[1]
    pushups += pu
    squats += sq
    await message.reply('кол-во прис:' + str(sq) + '\n' + 'кол-во отж:' + str(pu) + '\nобщее кол-во прис:' +
                        str(pushups) + '\nобщее кол-во отж' + str(squats))
